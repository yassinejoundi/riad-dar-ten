...
export function BookingSection() {
  return (
    <div id="before-you-book">
      ...
      <section className="bg-[#e7ddcf] py-24 md:py-32"><div className="mx-auto grid max-w-[1320px] gap-10 px-5 md:px-10 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="mb-4 font-sans text-xs font-semibold uppercase tracking-[.24em] text-terracotta">Ready when you are</p><h2 className="text-balance font-serif text-5xl font-light leading-[1.02] md:text-7xl">See what is available.</h2><p className="mt-5 max-w-xl text-pretty font-sans text-base leading-7 text-midnight-blue/75">The secure booking engine opens in a new tab so you can check dates without losing this page.</p></div><a href={bookingUrl} target="_blank" rel="noreferrer" className="group inline-flex min-h-14 items-center justify-between gap-8 bg-terracotta px-8 py-5 font-sans text-xs font-semibold uppercase tracking-[.18em] text-white hover:bg-deep-spice">Open booking <FontAwesomeIcon icon={faArrowRight} className="size-3 transition-transform group-hover:translate-x-1" /></a></div></section>
    </div>
  )
}
